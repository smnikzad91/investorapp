...
`{
  "status": true,
  "investor": {
    "id","email","first","last","phone",
    "isActive": bool,            // admin-approval gate, see Account status below
    "frozen": bool, "frozenReason": string|null,
    "tradingEnabled": bool,      // the investor's OWN on/off switch, independent of isActive
    "marginRatio": number,       // percent, e.g. 5 means 5%
    "profitSharePercent": number|null,  // admin-set, null until an admin configures it
    "debt": number
  },
  "hasApiKey": bool,
...
**Account status — compute a single banner from `investor`, in this exact priority order** (matches
the web dashboard's own logic, don't reorder):
1. `!isActive` → **Pending Approval**. Sub-text: if `hasApiKey` is true, "your API key is bound —
   an admin still needs to activate mirror trading"; if false, "bind your API key in Settings,
   then an admin will review and activate your account."
2. else if `frozen` → **Frozen**. Sub-text: "new mirrored trades are paused — you have an
   outstanding balance of $`debt`. Anything already open still closes normally. Contact the admin
   to settle it." (show `frozenReason` too if present)
3. else if `tradingEnabled === false` → **Trading Paused**. Sub-text: "you've paused new mirrored
   trades yourself — anything already open still closes normally. Turn it back on anytime in
   Settings."
4. else → **Active**. Sub-text: "mirror trading is on — every position opened on the main account
   opens on yours too, sized to your own margin ratio (`marginRatio`%)."
This is a computed banner, not a single API field — there's no `status: "active"` string to read,
derive it client-side from the four booleans/values above exactly as ordered.
...
### POST /api/investor/settings/trading-enabled
No body — this is a toggle, not a set-to-value call (it flips whatever the current value is).
Success: `{ "status": true, "tradingEnabled": bool }` — use the returned value to update the UI,
don't just assume the toggle worked and flip the local switch optimistically-and-forget; if the
call fails, revert the switch to its previous state.
Failure: `{ "status": false, "message": "..." }`
This is the investor's OWN pause switch (independent of the admin's `isActive` approval gate) —
turning it off stops *new* mirrored positions from opening; anything already open still closes
normally when the main account's does.

### POST /api/investor/settings/margin-ratio
Request: `{ "marginRatio": number }` — a percent, must be `> 0` and `<= 100`.
Success: `{ "status": true }`
Failure: `{ "status": false, "message": "Margin ratio must be a number between 0 and 100." }`
Controls how much of the investor's own available balance a mirrored trade risks: margin used =
`available balance × marginRatio / 100`, before the main account's own leverage is applied on top.
Pre-fill the input from GET /dashboard's `investor.marginRatio`.
...
3. **Dashboard** — a status banner at the top (see "Account status" under GET /dashboard above —
   Pending Approval / Frozen / Trading Paused / Active, in that exact priority order), then
   investor's wallet available/equity (from `assets`), open positions count, trade summary (win
   rate, total realized PNL, sample size vs total trades), and a prompt to bind an API key if
   `hasApiKey` is false. The banner is the first thing on this screen on the web version — keep it
   that prominent here too, it's the investor's main signal for "is my money actually being
   mirrored right now."
...
9. **Settings** — four independent things on one screen (matches the web layout):
   - **Profit Share** — read-only display of `profitSharePercent` (or "not configured yet — an
     admin sets this before your account can be activated" if null). Not editable here, admin-only.
   - **Trading Control** — a single switch bound to `tradingEnabled`, calls
     POST /settings/trading-enabled on toggle (see that endpoint above for the optimistic-update
     caveat). Label it something like "Mirror Trading" with the on/off sub-text from the Account
     status logic (Active vs Trading Paused wording).
   - **Margin Ratio** — numeric input (0–100, percent) pre-filled from `investor.marginRatio`,
     saved via POST /settings/margin-ratio on submit, with the "available × margin ratio, then the
     main account's leverage on top" explanation shown as a hint.
   - **Bind API Key** — form to paste key+secret, calls POST /bind-key, shows the success/failure
     message verbatim from the response (as before).
