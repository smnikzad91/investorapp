...
on the investor's profile when the alarm fires, in addition to the in-app notification. There's no
profile-update endpoint documented here yet for adding a phone number after registration if it was
left blank — check with the backend before building that part, don't assume a route exists.
...
6. **Alarms** — list of the investor's own price alarms (symbol, above/below, price, and whether
   SMS/call are on) with delete swipe/button, and a form to add one:
